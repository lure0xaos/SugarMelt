...
object StylesheetMain : IStylesheet {
    ...
    private val radiusBorders = 5.px
    ...
    private fun Style.cssI18n() {
        select(txt("select"), cls(CLASS_SELECT)) {
            backgroundColor(backgroundColor)
            color(bodyColor)
            borderRadius(radiusBorders)
            select("option") {
                backgroundColor(backgroundColor)
                color(bodyColor)
                and(":first-of-type") {
                    borderTopLeftRadius(radiusBorders)
                    borderTopRightRadius(radiusBorders)
                }
                and(":last-of-type") {
                    borderRadius(radiusBorders)
                    borderBottomLeftRadius(radiusBorders)
